...
internals.getUser = function (request, settings, addressOnly) {

    let user;

    if (!addressOnly && request.auth.isAuthenticated && request.auth.credentials.hasOwnProperty(settings.userAttribute)) {
        user = request.auth.credentials[settings.userAttribute];
    }
    else if (settings.trustProxy && request.headers['x-forwarded-for']) {
...
internals.userCheck = function (request, settings, done) {
    ...
    const ip = internals.getUser(request, settings, true);
    const user = internals.getUser(request, settings, settings.addressOnly);

    if ((settings.ipWhitelist.indexOf(ip) > -1) || (settings.userLimit === false)) {
